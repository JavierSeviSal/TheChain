...
from .models import (
    GameState, GamePhase, GameMode, CompetitionLevel,
    Card, CardType, Deck, Inventory, Tracks, MarketeerSlot,
    RECRUIT_TRAIN_TRACK, FoodItem,
)
...
class GameEngine:
    ...
    def new_game(self, modules: dict = None, optional_rules: dict = None,
                 mode: str = "full", language: str = "en") -> dict:
        ...
        self.state.log(f"Modules: {', '.join(k for k, v in self.state.modules.items() if v)}", "setup")
        self.state.log(f"Action Deck: {self.state.action_deck.size()} cards", "setup")

        return {"status": "ok", "message": "Game initialized. Place The Chain's first restaurant."}
    ...
    def advance_phase(self) -> dict:
        ...
        handlers = {
            ...
            GamePhase.GAME_OVER: lambda: {"status": "game_over", "message": "The game has ended."},
        }
    ...
    def process_input(self, input_data: dict) -> dict:
        ...
        if input_type == "first_restaurant_placed":
            ...
            return {"status": "ok", "message": "First restaurant placed. Begin Turn 1!",
            ...
            return {"status": "ok", "message": "Dinnertime resolved. Proceeding to Cleanup.",
                    "next_phase": "cleanup"}
            ...
            if self.state.bank_breaks >= 2:
                ...
                return {"status": "game_over", "message": "Second bank break! The game is over!"}
            return {"status": "ok", "message": f"Bank break #{self.state.bank_breaks} recorded."}

        elif input_type == "restaurant_placed":
            tile = input_data.get("tile", 1)
            self.state.restaurants.append({"tile": tile, "position": input_data.get("position", "")})
    ...
    def _do_first_turn(self) -> dict:
        ...
        self.state.pending_input = {
            ...
            "fields": [
                {"name": "tile", "label": "Map tile (1-9)", "label_es": "Casilla del mapa (1-9)",
    ...
    def _do_restructuring(self) -> dict:
        ...
        self.state.log(f"=== TURN {self.state.turn_number} — RESTRUCTURING ===", "phase")
    ...
    def _competition_adjustment(self) -> list[str]:
        ...
        if level == CompetitionLevel.HOT:
            ...
            self.state.log("Competition HOT → placed Warm card on top + under. Moved to WARM.", "restructuring")

        elif level == CompetitionLevel.WARM or self.state.optional_rules.get("aggressive_restructuring"):
            card = self.state.warm_deck.draw()
            if card:
                self.state.action_deck.place_under(card)
                msgs.append("WARM: Warm card placed under Action Deck.")
                self.state.log("Competition WARM → placed Warm card under deck.", "restructuring")
            ...
            if card:
                ...
                self.state.log("Competition COOL → placed Cool card under deck.", "restructuring")
            ...
            self.state.log("Competition COLD → placed Cool card on top + under. Moved to COOL.", "restructuring")
    ...
    def _check_resolve_competition(self, card: Card) -> Optional[str]:
        ...
        if card.card_type == CardType.WARM and level in (CompetitionLevel.WARM, CompetitionLevel.HOT):
            should_resolve = True
        elif card.card_type == CardType.COOL and level in (CompetitionLevel.COOL, CompetitionLevel.COLD):
            should_resolve = True

        # Optional rule: always resolve warm
        if card.card_type == CardType.WARM and self.state.optional_rules.get("aggressive_restructuring"):
            should_resolve = True
    ...
    def _resolve_competition_card(self, card: Card) -> str:
        ...
        if effect.effect_type == "expand_chain":
            if len(self.state.restaurants) < self.state.max_restaurants:
                self.state.pending_input = {
                    ...
                    "fields": [
                        {"name": "tile", "label": "Map tile", "label_es": "Casilla del mapa",
                         "type": "number", "min": 1, "max": 9, "default": effect.map_tile}
                    ]
                }
                msgs.append(f"EXPAND CHAIN → place restaurant on tile {effect.map_tile}")
            else:
            ...
            for slot in self.state.marketeer_slots:
                if slot.marketeer:
                    ...
                    slot.marketeer = None
                    slot.is_busy = False
            msgs.append(f"FIRE ALL EMPLOYEES: {', '.join(fired) if fired else 'none to fire'}.")
            for item in effect.inventory_loss_items:
                self.state.inventory.clear_item(item)
                msgs.append(f"INVENTORY LOSS: all {item} removed.")
        ...
        result = " | ".join(msgs)
        self.state.log(f"Resolved {card.card_type.value} card #{card.card_number}: {result}", "competition")
        return result
    ...
    def _do_recruit_train(self) -> dict:
        ...
        if not front_card_data or "front" not in front_card_data:
            ...
            return {"status": "ok", "message": "No front card available. Skipping Recruit & Train.",
                    "next_phase": "get_food"}
        ...
        self.state.log(f"Open slots: {open_slots}. Executed {len(active_actions)} actions.", "recruit_train")
        ...
        return {
            "status": "ok",
            "message": f"Recruit & Train complete ({open_slots} open slots). " + " | ".join(result_msgs),
    ...
    def _execute_recruit_action(self, action_data: dict) -> str:
        ...
        if requires and not self.state.modules.get(requires, False):
            if fallback:
                self.state.log(f"Module '{requires}' not in play. Getting {fallback} instead.", "recruit_train")
        ...
        if action_type == "recruit_marketeer":
            ...
            if new < 10 and "first_to_lower_prices" not in self.state.milestones_claimed:
            ...
            if new == 4:
                self.state.log("Waitresses reached 4! Recruit highest-ranking movie star.", "recruit_train")
    ...
    def _recruit_marketeer(self, name: str) -> str:
        ...
        if name == "Mass Marketeer":
            if not self.state.mass_marketeer:
                ...
                self.state.log(f"Mass Marketeer recruited (placed next to Track Mat).", "recruit_train")
        ...
        for slot in self.state.marketeer_slots:
            if slot.marketeer is None:
                slot.marketeer = name
                self.state.log(f"Recruited {name} to Marketeer slot {slot.slot_number}.", "recruit_train")
                return f"Recruited: {name} (slot {slot.slot_number})"

        # No open slots
        busy_count = sum(1 for s in self.state.marketeer_slots if s.is_busy)
        if busy_count >= 3:
            self.state.log(f"All marketeer slots full. Cannot recruit {name}.", "recruit_train")
    ...
    def _recruit_employee(self, name: str) -> str:
        ...
        if name == "Brand Director":
            ...
            for slot in self.state.marketeer_slots:
                if slot.marketeer is None:
                    ...
                    self.state.log(f"Brand Director placed in Marketeer slot {slot.slot_number}.", "recruit_train")
                    return f"Recruited: Brand Director (marketeer slot {slot.slot_number})"
        ...
        # Check milestones
        if self.state.tracks.get_open_slots() >= 2 and "first_to_train" not in self.state.milestones_claimed:
            self.state.milestones_claimed.append("first_to_train")
            self.state.log("Milestone claimed: First to Train Someone!", "milestone")

        if self.state.tracks.get_open_slots() >= 3 and "first_to_hire_3" not in self.state.milestones_claimed:
            self.state.milestones_claimed.append("first_to_hire_3")
            self.state.log("Milestone claimed: First to Hire 3 People in 1 Turn!", "milestone")
    ...
    def _do_get_food(self) -> dict:
        ...
        if not back_card or "back" not in back_card:
            ...
            return {"status": "ok", "message": "No back card. Skipping Get Food.",
        ...
        if demand_type == "specific":
            ...
            for item in food_items:
                if item in [fi.value for fi in FoodItem]:
                    if self.state.modules.get(item, True):  # Check module active
                        ...
                        self.state.log(f"+{food_amount * multiplier} {item}", "get_food")
            self.state.phase = GamePhase.MARKETING
            return {
                "status": "ok",
            ...
            self.state.pending_input = {
                ...
                "fields": [
                    {"name": "items_with_demand", "label": "Items with demand on map",
                     "label_es": "Items con demanda en el mapa",
                     "type": "multiselect",
                     "options": [fi.value for fi in FoodItem if self.state.modules.get(fi.value, True)]},
                    {"name": "most_demand_item", "label": "Item with MOST demand tokens",
    ...
    def _resolve_get_food(self, input_data: dict) -> dict:
        ...
        back = self.state.current_back_card.get("back", {}) if self.state.current_back_card else {}
        demand_type = back.get("demand_type", "most_demand")
        multiplier = back.get("multiplier", 1)
        food_amount = self.state.tracks.get_food_amount()
    ...
    def _do_marketing(self) -> dict:
        ...
        for slot in self.state.marketeer_slots:
            if slot.marketeer and not slot.is_busy:
                slot.is_busy = True
                campaigns.append(f"{slot.marketeer} (slot {slot.slot_number}) initiates marketing campaign")
                self.state.log(f"{slot.marketeer} starts a marketing campaign.", "marketing")

        if self.state.mass_marketeer:
            campaigns.append("Mass Marketeer runs additional marketing campaign")
            self.state.log("Mass Marketeer runs additional marketing campaign phase.", "marketing")

        # Check for stars from current card
        self.state.phase = GamePhase.DEVELOP
        msg = "Marketing: " + (" | ".join(campaigns) if campaigns else "No new campaigns.")
    ...
    def _do_develop(self) -> dict:
        ...
        self.state.phase = GamePhase.LOBBY
        return {"status": "ok", "message": "No DEVELOP star. Skipping.", "next_phase": "lobby"}
    ...
    def _do_lobby(self) -> dict:
        ...
        self.state.phase = GamePhase.EXPAND_CHAIN
        return {"status": "ok", "message": "No LOBBY star. Skipping.", "next_phase": "expand_chain"}
    ...
    def _do_expand_chain(self) -> dict:
        ...
        stars = getattr(self.state, '_pending_stars', [])
        front = self.state.current_front_card.get("front", {}) if self.state.current_front_card else {}
        map_tile = front.get("map_tile", 1)

        if "expand_chain" in stars and len(self.state.restaurants) < self.state.max_restaurants:
            self.state.pending_input = {
                "type": "restaurant_placed",
                ...
                "fields": [
                    {"name": "tile", "label": "Map tile placed on", "label_es": "Casilla donde se coloca",
                     "type": "number", "min": 1, "max": 9, "default": map_tile}
                ]
            }
            self.state.phase = GamePhase.WAITING_FOR_INPUT
            return {
        ...
        self.state.phase = GamePhase.DINNERTIME
        return {"status": "ok", "message": "No expansion. Proceeding to Dinnertime.", "next_phase": "dinnertime"}
    ...
    def _continue_after_stars(self) -> dict:
        ...
        self.state.phase = GamePhase.DINNERTIME
        return {"status": "ok", "message": "Proceeding to Dinnertime.", "next_phase": "dinnertime"}
    ...
    def _do_dinnertime_prompt(self) -> dict:
        ...
        self.state.pending_input = {
            ...
            "fields": [
                {"name": "chain_earned", "label": "Chain earned ($)",
                 "label_es": "La Cadena ganó ($)", "type": "number", "min": 0},
                {"name": "player_earned", "label": "You earned ($)",
                 "label_es": "Tú ganaste ($)", "type": "number", "min": 0},
            ]
        }
        self.state.phase = GamePhase.WAITING_FOR_INPUT
        return {
    ...
    def _do_cleanup(self) -> dict:
        ...
        if "Kimchi Master" in [s.marketeer for s in self.state.marketeer_slots] or \
           "Kimchi Master" in self.state.employee_pile:
        ...
        if shuffle_needed:
            ...
            while top and top.card_type in (CardType.WARM, CardType.COOL):
                self.state.action_deck.shuffle()
                self.state.log("Competition card on top after shuffle — reshuffling.", "cleanup")
                top = self.state.action_deck.peek()
        ...
        result_msg = "Cleanup complete: " + (" | ".join(msgs) if msgs else "no adjustments")
        self.state.log(f"Turn {self.state.turn_number - 1} complete. Starting Turn {self.state.turn_number}.", "phase")
    ...
    def undo(self) -> dict:
        ...
        if "recruit_train" in tracks_data:
            self.state.tracks.recruit_train.position = tracks_data["recruit_train"]["position"]
        if "price_distance" in tracks_data:
            self.state.tracks.price_distance.position = tracks_data["price_distance"]["position"]
        if "waitresses" in tracks_data:
            self.state.tracks.waitresses.position = tracks_data["waitresses"]["position"]
        if "competition" in tracks_data:
            self.state.tracks.competition = CompetitionLevel(tracks_data["competition"]["level"])
