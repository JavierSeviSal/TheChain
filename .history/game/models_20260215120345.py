...
class ActionSlot:
    ...
    fallback_food: Optional[str] = None  # Food item if action can't be taken (module not in use)
...
class CardFront:
    ...
    stars: list[str] = field(default_factory=list)  # "coffee_shop", "expand_chain", "develop", "lobby"
...
class CardBack:
    ...
    food_items: list[str] = field(default_factory=list)  # Specific food items or empty for demand-based
...
class Card:
    ...
    card_number: int  # Number printed on the card (1-20 for action, 1-12 for competition)
    ...
    def to_dict(self) -> dict:
        ...
        if self.front:
            d["front"] = {
                "actions": [
                    {"slot": a.slot_number, "type": a.action_type, "target": a.target,
                     "fallback_food": a.fallback_food, "requires_module": a.requires_module}
...
class TrackMarker:
    ...
    def to_dict(self) -> dict:
        d = {"name": self.name, "position": self.position,
             "min": self.min_pos, "max": self.max_pos}
        if self.labels:
...
class Tracks:
    ...
    recruit_train: TrackMarker = field(default_factory=lambda: TrackMarker(
        name="recruit_train", position=1, min_pos=1, max_pos=7,
        labels={1: "1 Open Slot", 2: "2 Open Slots", 3: "2 Open Slots",
                4: "3 Open Slots", 5: "3 Open Slots", 6: "4 Open Slots", 7: "4 Open Slots"}
    ))
    price_distance: TrackMarker = field(default_factory=lambda: TrackMarker(
        name="price_distance", position=10, min_pos=6, max_pos=10,
    ))
    waitresses: TrackMarker = field(default_factory=lambda: TrackMarker(
        name="waitresses", position=0, min_pos=0, max_pos=4,
    ))
    competition: CompetitionLevel = field(default_factory=lambda: CompetitionLevel.NEUTRAL)
...
class Inventory:
    ...
    def to_dict(self) -> dict:
        return {
            item: {"top": v["top"], "bottom": v["bottom"], "total": v["top"] + v["bottom"]}
...
class GameState:
    ...
    optional_rules: dict = field(default_factory=lambda: {
        "hard_choices": False,
    ...
    # Marketeers
    marketeer_slots: list[MarketeerSlot] = field(default_factory=lambda: [
        MarketeerSlot(1), MarketeerSlot(2), MarketeerSlot(3)
    ])
