...
from .models import (
    ...
    CardType, Card, CardFront, CardBack, ActionSlot, CleanupAction,
    CompetitionEffect, Deck, Tracks, TrackMarker, Inventory,
    MarketeerSlot, FoodItem,
)
...
def save_game(state: GameState, slot_name: str = "autosave") -> dict:
    ...
    return {"status": "ok", "message": f"Game saved to '{slot_name}'.", "filepath": filepath}
    ...
    for filename in sorted(os.listdir(SAVES_DIR)):
        if filename.endswith(".json"):
            ...
            try:
                ...
                saves.append({"slot_name": filename[:-5], "date": "corrupted", "turn": 0})
    return saves
