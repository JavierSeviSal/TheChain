...
def _detect_grid(arr):
    ...
    Returns (h_lines, v_lines) where each is a list of (start_row, end_row)
    or (start_col, end_col) tuples for the border lines.
    Only the 3 most prominent lines in each direction are returned.
    """
    h, w = arr.shape[:2]
    ...
    # A border line should span a significant portion of the page
    h_threshold = w * 0.25
    v_threshold = h * 0.25
    ...
    # We expect exactly 3 lines in each direction (top/mid/bottom, left/mid/right).
    # If there are more, keep only the 3 with the highest total gray count.
    def _top3(groups, counts):
        if len(groups) <= 3:
            return sorted(groups)
        scored = []
        for s, e in groups:
            scored.append((counts[s:e + 1].sum(), (s, e)))
        scored.sort(reverse=True)
        return sorted([g for _, g in scored[:3]])

    h_lines = _top3(h_lines, row_count)
    v_lines = _top3(v_lines, col_count)
...
def extract_cards():
    ...
    for page_idx in range(len(doc)):
        ...
        for i, (x1, y1, x2, y2) in enumerate(regions):
            ...
            print(f"  Page {page_idx+1:2d} → {filename}  ({cropped.width}×{cropped.height})")
